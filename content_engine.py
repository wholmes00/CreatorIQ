...
# --- Load .env file (single source of truth for all API keys) ---
try:
...
# --- Load system config ---
_config_path = Path(__file__).resolve().parent / "system_config.json"
try:
...
USE CASE RULE — SCENARIO DIVERSITY
  Each of the 2 hero videos must showcase a DIFFERENT real use case.
  ...
  - Hero 1: An emotional/personal moment OR discovery angle
  - Hero 2: A different angle — warning, value, social, or lifestyle moment
...
{chr(10).join(all_hook_lines)}
  ...
  YOUR HOOKS MUST:
    ...
    - Use a different APPROACH for each hero (pick 2 different ones):
  ...
  WRITE YOUR 2 HOOKS FIRST. Then check each one against the top 5 above.
...
{chr(10).join(vd_lines)}
  ...
  REQUIRED MIX for 2 hero videos:
    - Both heroes MUST be talking_head format with MIX of on-camera and voiceover lines
      (data shows pure on-camera underperforms mixed delivery)
    - Each hero should use a DIFFERENT voice delivery style
    - The 2 heroes provide the creator's face-to-camera connection

  REQUIRED MIX for 10 remix videos (assembled by editor from shoot material):
    - 7 voiceover + b-roll remixes (creator records VO audio, editor layers over b-roll)
    - 3 text-only remixes (no voice, music + on-screen text + b-roll)
    - The SHOOT GUIDE only includes the 7 voiceover scripts. The 3 text-only remixes
      are editing-only decisions and appear ONLY in the Edit Guide.""")
...
RULE 6 — VARIETY THROUGH ANGLE & EXECUTION
  ...
  ANGLE VARIETY (each hero approaches the product from a different life angle):
    - Hero 1: Personal story / emotional connection / discovery
    - Hero 2: Warning / emotional impact / deeper angle
  ...
  TONE VARIETY (energy shifts across the 2 heroes):
    - Hero 1: higher energy (excited, amazed, discovery)
    - Hero 2: different tone (emotional, warning, conversational)
...
HOW TO USE THIS:
- If the product fits a holiday gifting angle, make 1 of the 2 hero scripts holiday-themed
...
- DO NOT force holiday content. If the fit isn't there, all scripts should be evergreen.

SPLIT GUIDELINE (when holiday fits):
- 1 hero script: EVERGREEN (works year-round)
- 1 hero script: HOLIDAY-THEMED (tied to the active holiday)
- 7-8 remixes: EVERGREEN
- 2-3 remixes: HOLIDAY-THEMED
"""
...
For each hero, the hook is the first line. Build the rest of the script
...
=== YOUR TASK ===
Generate the SHOOT GUIDE JSON only.

CRITICAL — LOCKED FORMAT (do NOT deviate from this structure):

{{"productName":"...","totalVideos":12,
"onCameraLines":{{
  "heroes":[
    {{"title":"Hero 1 — Concept Name",
      "lines":[
        {{"type":"ON CAMERA","text":"[LOCKED HOOK — copy exactly]"}},
        {{"type":"ON CAMERA","text":"Second line..."}},
        {{"type":"VOICEOVER","text":"Third line..."}}
      ]
    }},
    {{"title":"Hero 2 — Concept Name",
      "lines":[...]
    }}
  ]
}},
...
"remixVoiceovers":[
  {{"title":"Remix 1 — Concept Name","script":"Full paragraph voiceover script. Multiple sentences. This is what the creator records as audio-only, to be layered over b-roll by the editor."}},
  {{"title":"Remix 2 — Concept Name","script":"Another full paragraph..."}}
]
}}

REQUIREMENTS:
- EXACTLY 2 hero scripts, each starting with its LOCKED hook as the first line
- 7-8 lines per hero script, 140-220 words, mix of ON CAMERA and VOICEOVER
...
- EXACTLY 15 b-roll shots (FILMABLE ONLY — no text overlays, no graphics, no post-production elements)
- EXACTLY 7 remix voiceover scripts — each is a FULL PARAGRAPH (3-6 sentences, 40-80 words) that the creator records as audio-only. These are NOT short taglines. They are complete voiceover narrations.
...
- Do NOT include propsAndSetup, locations, or wardrobe — the creator decides that
...
def generate_content_plan_v2(product_brief, product_category=None, web_research="",
    ...
    print(f"\n  HOOK RANKINGS:")
    for item in ranked:
        n_heroes_display = CONFIG.get("content_rules", {}).get("hero_videos", 2)
        marker = " <<<" if item["rank"] <= n_heroes_display else ""
        print(f"    #{item['rank']:2d} (score {item['score']:3d}): \"{item['text'][:60]}\"{marker}")

    # Pick top N hooks with diversity enforcement (N = number of hero videos from config)
    n_heroes = CONFIG.get("content_rules", {}).get("hero_videos", 2)
    diverse_top = select_diverse_top_n(ranked, n=n_heroes, similarity_threshold=0.3)
    top_hooks = [item["text"] for item in diverse_top]
    print(f"\n  TOP {n_heroes} HOOKS (diversity-checked):")
