...
# ── Load .env (single source of truth for all API keys) ────────
...
def run_pipeline(args):
    ...
    # Validate shoot guide structure (supports both old and new format)
    if "onCameraLines" in shoot_guide:
        ths = shoot_guide.get("onCameraLines", {}).get("heroes", [])
    else:
        ths = shoot_guide.get("masterShotList", {}).get("talkingHeadShots", [])
    brolls = shoot_guide.get("brollShots", []) or shoot_guide.get("masterShotList", {}).get("brollShots", [])
    vos = shoot_guide.get("remixVoiceovers", []) or shoot_guide.get("masterShotList", {}).get("voiceoverAudio", [])
...
def build_edit_guide_prompt(shoot_guide_data, product_research, product_brief):
    ...
    LOCKED FORMAT: 2 hero videos + 10 remix videos.
    Hero structure: Hook, Duration, Audio, Script, B-Roll Used, On-Screen Text.
    Remix structure: Hook, Duration, Audio, Script (optional), B-Roll Used, On-Screen Text.
    NO CTA strategy sections. NO upload details. NO visual timeline.
    """
...
YOUR TASK: Create an Edit Guide JSON that tells the editor exactly how to assemble 2 hero videos + 10 remix videos from the shot material above.

CRITICAL — LOCKED FORMAT. Do NOT add any sections or fields beyond what is shown below.
...
{{
  ...
  "heroVideos": [
    {{
      "title": "Hero 1 — [concept from shoot guide]",
      "hook": "the hook line exactly as in shoot guide",
      "duration": "35-40s",
      "audio": "upbeat_trending",
      "script": [
        {{"type": "ON CAMERA", "text": "first line (the hook)"}},
        {{"type": "ON CAMERA", "text": "second line"}},
        {{"type": "VOICEOVER", "text": "voiceover line"}},
        {{"type": "ON CAMERA", "text": "next on-camera line"}}
      ],
      "brollUsed": [
        {{"time": "0:08-0:11", "label": "B1", "description": "Close-up of the product"}},
        {{"time": "0:11-0:14", "label": "B2", "description": "Hands holding product"}}
      ],
      "onScreenText": [
        {{"time": "0:00-0:08", "text": "hook text with emoji"}},
        {{"time": "0:08-0:19", "text": "feature callout"}},
        {{"time": "0:19-0:24", "text": "social proof text"}},
        {{"time": "0:24-0:40", "text": "CTA text with emoji"}}
      ]
    }}
  ],
  "remixVideos": [
    {{
      "title": "Remix 1 — [concept name]",
      "hook": "short hook text with emoji",
      "duration": "18-22s",
      "audio": "soft_trending",
      "script": [
        {{"type": "VOICEOVER", "text": "first voiceover line"}},
        {{"type": "VOICEOVER", "text": "second voiceover line"}}
      ],
      "brollUsed": [
        "B1 Close-up of product",
        "B2 Hands holding product",
        "B7 Lifestyle shot"
      ],
      "onScreenText": [
        {{"time": "0:00-0:22", "text": "hook text with emoji"}}
      ]
    }}
  ]
}}
...
RULES:
- EXACTLY 2 hero videos matching the 2 heroes from the shoot guide
- EXACTLY 10 remix videos: 7 with voiceover scripts + 3 text-only (no script field for text-only)
- Hero scripts MUST copy the lines exactly from the shoot guide — same words, same ON CAMERA/VOICEOVER tags
- B-Roll Used for heroes MUST have timestamps showing when each b-roll clip appears
- B-Roll Used should be 3-4 clips per hero — NOT every b-roll shot. Heroes are mostly face-on-camera.
- The face:b-roll ratio should be roughly 60:40 — most of the hero is the creator talking on camera
- On-Screen Text for heroes MUST have timestamp ranges covering the full video duration
- Remix b-roll lists reference actual B# labels from the shoot guide (5-6 per remix)
- The 7 voiceover remixes use the scripts from the shoot guide's remixVoiceovers section, broken into individual VOICEOVER lines
- The 3 text-only remixes have NO script field — just brollUsed + onScreenText
- Audio styles: upbeat_trending, soft_trending, soft_emotional, cozy_lo-fi, confident_trending, honest_trending, punchy, etc.
- Do NOT include: ctaStrategy, visualTimeline, uploadDetails, angle, format, shotAssembly, or cta fields
